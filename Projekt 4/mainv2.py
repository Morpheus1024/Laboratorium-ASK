...
def start_server_window():
    def accept_client():
        ...
        print("Serwer nasłuchuje na porcie 5555")
        ...
        while True:
            try:
                ...
                while len(message_buffer) >= 8:  # Sprawdź, czy wystarczająco danych do dekodowania
                    ...
                    # sprawdzanie czy wiadomosc zawiera wulgaryzmy (wez pod uwage zmiane wielkosci liter)
                    with open('grubianstwa.txt', 'r') as file:
                        for line in file:
                            if (line.strip() == decoded_message.lower() or line.strip() == decoded_message.upper() or line.strip() == decoded_message):
