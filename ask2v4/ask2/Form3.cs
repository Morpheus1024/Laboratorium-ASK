    ...
    public partial class Form3 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (this.czy_pierwsza > 1)
            {
                ...
                {
                    timer1.Stop();
                    MessageBox.Show($"Skuteczność wynosi: {(float)this.licznik_dobrych_ruchow / (float)this.wystapienia * 100:F2}% a średni czas reakcji wynosi: {this.sredni_czas/this.licznik_dobrych_ruchow:F3} sek."); 
                    this.Close();

                    licznik2 = (float)this.licznik_dobrych_ruchow / (float)this.wystapienia * 100;
                    czas2 = this.sredni_czas / this.licznik_dobrych_ruchow;
                    this.OknoRodzic.label3.Text = this.licznik2.ToString();
                    this.OknoRodzic.label4.Text = this.czas2.ToString();
                }
            }
        }
