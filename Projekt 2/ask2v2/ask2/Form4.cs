...
namespace ask2
{
    public partial class Form4 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            else if (licznik_czasu > 11 && licznik_czasu < 31)
            {
                switch (wiersze)
                {
                    case 1:
            ...
            {
                timer1.Stop();
                MessageBox.Show($"ilość dobrych ruchów:" + this.licznik_dobrych_ruchow + $"; skuteczność wyniosła:{(float)this.licznik_dobrych_ruchow / 20 * 100}%" +
                        $"; średni czas reakcji wyniósł:{((float)this.sredni_czas) / this.iloscDobrychRuchow} sekundy");
            }
        }
